    ...
    function sortProposals(uint256 steps) public returns (bool) {
        ...
        while (sortedWords < proposalsBeingSorted.length) {
            ...
            if (savedIndex > proposalsBeingSorted.length) {
                sortedWords = proposalsBeingSorted.length - swap;
                swaps = 0;
                savedIndex = 1;
        }
        else {
            Proposal memory prebObj = proposalsBeingSorted[savedIndex - 1];
            if (uint256(prebObj.name) > uint256(proposalsBeingSorted[savedIndex].name)){
